...
import { handleCreateItem, handleFetchAllItems } from "@/app/manager/itemManager";
...
const ItemEdit = () => {
  ...
  const { data: items } = useAppSelector((state) => state.item);
  ...
  const [mappingType, setMappingType] = useState<"single" | "multiple">("single");
  const [assignments, setAssignments] = useState<Assignment[]>([]);
  const [isQtyDialogOpen, setIsQtyDialogOpen] = useState(false);
  const [pendingMapping, setPendingMapping] = useState<{pathIds: number[], fullPath: string} | null>(null);
  ...
  useEffect(() => {
    const loadData = async () => {
      await dispatch(handleFetchBins({ warehouseid: 1 }));
  ...
  useEffect(() => {
    ...
    if (item) {
      setFormData(item);
      // Construct existing assignments if possible
      if (item.location && item.location.length > 0) {
        const existing: Assignment[] = item.location.map((locId, idx) => ({
          path: `Existing Location Assignment #${idx + 1}`,
          pathIds: [locId, item.floor?.[idx] || 0, 0, 0, item.device?.[idx] || 0],
          // In real scenarios, quantity per location would come from DB. 
          // For now, we'll split the total or assume it's set.
          qty: idx === 0 ? item.open_quantity : 0 
        }));
  ...
  const handleSubmit = async () => {
    ...
    const success = await dispatch(handleCreateItem(formData, Number(id)));
    if (success) {
  ...
  if (isLoading) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center gap-4 text-slate-400">
        ...
        <p className="font-black uppercase tracking-widest text-[10px]">Loading Item Identity...</p>
      </div>
    );
  }
                      ...
                      >
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            const totalQty = newAssignments.reduce((sum, a) => sum + a.qty, 0);
        ...
        <DialogContent className="sm:max-w-[400px] rounded-[2rem] p-0 overflow-hidden border-0 shadow-2xl">
          <div className="bg-indigo-600 p-8 text-white space-y-2">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-black uppercase tracking-tight">Allocate Quantity</h2>
            </div>
          ...
          <div className="p-8 space-y-8 bg-white">
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
              <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 block">Selected Path</Label>
              <p className="text-sm font-black text-slate-900 break-words">{pendingMapping?.fullPath}</p>
            </div>
...
const BinSelectionSystem = ({
  ...
  const isReady = selections[0] !== 0 && selections[4] !== 0;
