...
import { fetchOutwardRequests, setGenerating } from "@/app/store/requestSlice";
...
const OutwardRequest = () => {
  ...
  const updateQty = (id: number, val: number) => {
    setLocalData((prev) =>
      prev.map((r) =>
        r.id === id
          ? { ...r, pickQty: Math.max(0, Math.min(val, r.availableQty)) }
          : r,
      ),
    );
  };
  ...
  const executeGenerate = async () => {
    ...
    try {
      ...
      if (response.data.status) {
        toast.success("Outward Picklist successfully created");
        setSelected(new Set());
                  ...
                  <div className="flex items-center gap-4 mb-2">
                    ...
                    <div>
                      <DialogTitle className="text-2xl font-black tracking-tight">
                        Confirm Outward Picklist
                      </DialogTitle>
                      <p className="text-slate-400 text-sm font-medium">
                        Verify picking quantities for the selected{" "}
                  ...
                  <div className="flex gap-4">
                    ...
                    >
                      {isGenerating ? (
                        ...
                        <>
                          <Truck className="h-5 w-5 mr-2" /> Create Picklist
                        </>
                      )}
                    </Button>
                  </div>
