...
const PicklistDispatchPage = () => {
  ...
  const picklistData = dispatchData.filter(
    (row: any) =>
      ...
      row.grpo_doc_entry === undefined ||
      row.grpo_doc_entry === "",
  );

  const dateFilteredData = useMemo(() => {
    return picklistData.filter((row: any) => {
  ...
  const groupedWarehouses = useMemo(() => {
    ...
    dateFilteredData.forEach((row: any) => {
      // Use composite key including whscode and outward_header_id (since GRPO is null for picklist)
      const uniqueKey = `${row.whscode}_${row.item_code}_${row.outward_header_id}_${row.qty}`;
    ...
    <div className="flex flex-col gap-4 animate-in fade-in duration-500 bg-[#f8fafc]/50 p-4 min-h-screen">
      ...
      <div className="shrink-0 flex items-center justify-between px-8 py-6 bg-white border-1 border-slate-50 shadow-lg shadow-slate-400 rounded-[32px] mb-2 w-full">
        <div className="flex items-center gap-6">
          <div className="w-14 h-14 rounded-2xl bg-sky-600 flex items-center justify-center shadow-lg shadow-sky-200 ring-4 ring-sky-50">
            <Truck className="w-7 h-7 text-white" />
          </div>
          <div className="space-y-1">
            <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase">
              Picklist <span className="text-sky-600">Dispatch</span>
            </h1>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em]">
              Non-GRPO Dispatch Monitoring
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="relative group">
            ...
            <Input
              ...
              className="pl-12 h-12 w-80 rounded-2xl bg-slate-50 border-1 border-slate-200 shadow-lg shadow-slate-300 focus:bg-white focus:ring-4 focus:ring-sky-50 transition-all text-sm font-black"
              value={searchTerm}
            ...
            <PopoverTrigger asChild>
              <Button
                ...
                className={`h-12 px-6 rounded-2xl border-1 font-black text-[10px] uppercase tracking-widest flex gap-2 active:scale-95 transition-all ${
                  fromDate || toDate
                    ? "bg-sky-50 border-sky-300 text-sky-600 shadow-lg shadow-sky-100"
              ...
              <tbody className="bg-white">
                {loading ? (
                  <tr>
                    <td colSpan={7} className="py-32 text-center">
                      <div className="flex flex-col items-center justify-center gap-4">
                        <Loader2 className="h-14 w-14 text-sky-600 animate-spin" />
                  ...
                  paginatedData.map((whs, idx) => (
                    <tr
                      key={whs.whscode}
                      className="hover:bg-sky-50/30 transition-all duration-300 border-b border-slate-100 last:border-0 group cursor-pointer"
                      onClick={() =>
                        navigate(
                          `/transactions/picklist/dispatch/warehouse/${whs.whscode}`,
                        )
                      }
                    >
                      ...
                      <td className="px-8 py-5 text-right pr-12">
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-10 w-10 rounded-2xl bg-slate-200 text-slate-700 shadow-lg shadow-slate-400 hover:bg-sky-600 hover:text-white transition-all shadow-sm active:scale-95"
                          onClick={(e) => {
                            e.stopPropagation();
                            navigate(
                              `/transactions/picklist/dispatch/warehouse/${whs.whscode}`,
        ...
        <div className="flex items-center justify-between px-10 py-6 bg-white rounded-[32px] shadow-xl shadow-slate-100">
          <div className="flex items-center gap-3">
            ...
            <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">
              Showing page <span className="text-sky-600">{page}</span> of{" "}
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              className="h-12 px-8 rounded-2xl border-0 bg-slate-50 hover:bg-sky-50 text-sky-600 font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 disabled:opacity-30"
            >
...
export default PicklistDispatchPage;
