...
import { fetchOutwardHistory } from "@/app/store/historySlice";
...
const OutwardHistory = () => {
  const navigate = useNavigate();
  const dispatch = useAppDispatch();
  const { data, loading, totalCount } = useAppSelector(
    (state) => state.history.outward,
  );

  const [page, setPage] = useState(1);

  useEffect(() => {
    dispatch(fetchOutwardHistory({ page, size: PAGE_SIZE }));
  }, [dispatch, page]);
            ...
            <TableBody>
              {data.length === 0 && !loading ? (
                <TableRow>
                  ...
                  >
                    No outward history records found
                  </TableCell>
                </TableRow>
              ) : (
                data.map((row) => {
                    ...
                    >
                      ...
                      <TableCell className="text-right pr-6">
                        <Button
                          ...
                          onClick={() =>
                            navigate(`/transactions/putaway-outward/${row.id}`)
                          }
                        >
